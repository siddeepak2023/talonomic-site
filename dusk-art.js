...
/* ---------- BUST (sentinel) — head dotwork, raster-then-stipple ----------
   The CC0 MakeHuman head mesh is shaded into a small offscreen luminance
   buffer (Lambert key + silhouette rim), lightly blurred so no facet or
   wireframe survives — nose/brow/chin read as soft shading — then rendered
   as a dense fine-grain density field: the hero-terrain particle language.
   Head + neck only; the neck dissolves into dust at the base. */
/* CC0 head mesh — decimated from MakeHuman base.obj (makehumancommunity, explicit CC0) */
...
function headModel(){
  if (headModel.__m) return headModel.__m;
  var V = HEAD_V, F = HEAD_F, i;
  var nf = F.length / 3;
  var fn = new Array(nf), fz = new Array(nf), fa = new Array(nf);
  for (i = 0; i < nf; i++) {
    var a = F[i*3]*3, b = F[i*3+1]*3, c = F[i*3+2]*3;
    var ax=V[a]/1000, ay=V[a+1]/1000, az=V[a+2]/1000;
    var bx=V[b]/1000, by=V[b+1]/1000, bz=V[b+2]/1000;
    var cx=V[c]/1000, cy=V[c+1]/1000, cz=V[c+2]/1000;
  ...
  headModel.__m={fn:fn,fz:fz,fa:fa,nf:nf};
  return headModel.__m;
}
function drawBust(cv){
  /* Raster-then-stipple: mesh shaded into a small offscreen luminance buffer
     (Lambert key + silhouette rim), lightly blurred so facets die but the
     nose/brow/eye survive as soft shading, then rendered as a dense fine-grain
     density field — hero-terrain particle language. Head + short neck. */
  var p = prepArt(cv); if (!p) return;
  var c2 = p.c2, w = p.w, h = p.h, fg = p.fg, sg = p.sg;
  var M = headModel(), V = HEAD_V, F = HEAD_F, i, x, y;
  var HEAD_MAX_Y = 640; /* head + short neck; shoulders dropped */

  /* placement: bust fills the height; head (x -0.16..0.23, centered 0.037)
     sits centered; shoulders (±0.57) bleed off the band edges by design */
  var S = h * 1.18, oy = h * 0.05; /* full crown->neck fits, not clipped */
  var ox = w * 0.5 + 0.037 * S; /* mirror: px = ox - x·S, so +offset centers the head */
  /* raster buffers */
  var RW = Math.max(90, Math.min(190, w * 0.55 | 0));
  var RH = Math.max(120, Math.min(240, h * 0.55 | 0));
  var rx = RW / w, ry = RH / h;
  var lum = new Float32Array(RW * RH);
  var cov = new Float32Array(RW * RH);
  var dep = new Float32Array(RW * RH); /* z-buffer, larger z = nearer */
  for (i = 0; i < dep.length; i++) dep[i] = -1e9;

  var L = [-0.34, -0.58, 0.62]; /* key: upper-front-left */
  var ll = Math.sqrt(L[0]*L[0]+L[1]*L[1]+L[2]*L[2]); L=[L[0]/ll,L[1]/ll,L[2]/ll];
  ...
  for (i = 0; i < M.nf; i++) {
    ...
    /* project (mirrored x) into raster space */
    ...
    var zc=(V[a+2]+V[b+2]+V[c+2])/3;
    var lit = Math.max(0, n[0]*L[0]+n[1]*L[1]+n[2]*L[2]);
    var shade = 0.30 + 0.70 * Math.pow(lit, 1.15); /* high ambient — even luminous fill, no socket holes */
    ...
    for (y = y0; y <= y1; y++) for (x = x0; x <= x1; x++) {
      ...
      var idx=y*RW+x;
      if (zc>dep[idx]) { dep[idx]=zc; lum[idx]=shade; cov[idx]=1; }
    }
  }

  /* smooth: 3 normalized box-blur passes kill facet edges (mannequin skin) */
  var tmpL = new Float32Array(RW * RH), tmpC = new Float32Array(RW * RH), pass;
  for (pass = 0; pass < 2; pass++) {
    for (y = 0; y < RH; y++) for (x = 0; x < RW; x++) {
      ...
      for (var dy=-1; dy<=1; dy++) for (var dx=-1; dx<=1; dx++) {
        ...
        sL+=lum[yy*RW+xx]; sC+=cov[yy*RW+xx]; cnt++;
      }
      tmpL[y*RW+x]=sL/cnt; tmpC[y*RW+x]=sC/cnt;
    }
    lum.set(tmpL); cov.set(tmpC);
  }

  /* rim: brightness lifts where coverage falls off (silhouette), biased up */
  for (y = 0; y < RH; y++) for (x = 0; x < RW; x++) {
    var idc=y*RW+x, cvg=cov[idc];
    if (cvg < 0.03) continue;
    var edge = 1 - Math.min(1, cvg * 1.15);          /* 0 core -> 1 silhouette */
    var topBias = 1 - (y / RH) * 0.35;                /* crown glows hardest */
    lum[idc] = Math.min(1.35, lum[idc] + Math.pow(edge, 1.15) * 1.5 * topBias);
  }
  /* diffuse the rim into a soft luminous band (no hard contour line) */
  for (pass = 0; pass < 2; pass++) {
    for (y = 0; y < RH; y++) for (x = 0; x < RW; x++) {
      var sL2=0, cnt2=0;
      for (var dy2=-1; dy2<=1; dy2++) for (var dx2=-1; dx2<=1; dx2++) {
        var yy2=y+dy2, xx2=x+dx2;
        if (yy2<0||yy2>=RH||xx2<0||xx2>=RW) continue;
        sL2+=lum[yy2*RW+xx2]; cnt2++;
      }
      tmpL[y*RW+x]=sL2/cnt2;
    }
    lum.set(tmpL);
  }

  /* soft glow behind the crown (under the grain) */
  var gx = w * 0.5, gyy = h * 0.30;
  var grad = c2.createRadialGradient(gx, gyy, 0, gx, gyy, h * 0.52);
  grad.addColorStop(0, fg); grad.addColorStop(1, "rgba(0,0,0,0)");
  c2.globalAlpha = 0.05; c2.fillStyle = grad;
  c2.fillRect(0, 0, w, h);

  /* stipple: dense fine grain, density & alpha follow the smooth luminance */
  var FADE = h * 0.80;
  var A = Math.min(150000, (w * h * 0.62) | 0);
  for (i = 0; i < A; i++) {
    var ux = hash2(i, 3), uy = hash2(i, 7);
    var px = ux * w, py = uy * h;
    var sx = Math.min(RW - 1.001, ux * RW), sy = Math.min(RH - 1.001, uy * RH);
    var xi = sx | 0, yi = sy | 0, fx2 = sx - xi, fy2 = sy - yi;
    var i00 = yi * RW + xi, i10 = i00 + 1, i01 = i00 + RW, i11 = i01 + 1;
    if (i11 >= lum.length) continue;
    var Lv = lum[i00]*(1-fx2)*(1-fy2) + lum[i10]*fx2*(1-fy2) + lum[i01]*(1-fx2)*fy2 + lum[i11]*fx2*fy2;
    var Cv = cov[i00]*(1-fx2)*(1-fy2) + cov[i10]*fx2*(1-fy2) + cov[i01]*(1-fx2)*fy2 + cov[i11]*fx2*fy2;
    if (Cv < 0.04) continue;
    var grain = 0.62 + 0.72 * fbm(ux * 7.5, uy * 7.5, 2.6);
    var keep = Math.min(1, Cv * 1.25) * (0.06 + 0.94 * Math.pow(Math.min(1, Lv), 1.35)) * grain;
    if (hash2(i, 11) > keep) continue;
    var fd = py < FADE ? 1 : Math.max(0, 1 - (py - FADE) / (h - FADE));
    if (fd <= 0) continue;
    var spark = hash2(i, 31) > 0.9986;
    c2.fillStyle = spark ? sg : fg;
    c2.globalAlpha = (spark ? 0.9 : Math.min(0.95, 0.22 + 0.7 * Lv) * (0.6 + 0.4 * hash2(i, 13))) * fd;
    var szb = Lv > 0.85 && hash2(i, 17) > 0.965 ? 1.8 : 1.05;
    c2.fillRect(px, py, szb, szb);
  }

  /* ambient dust around the figure */
  for (i = 0; i < 620; i++) {
    c2.fillStyle = fg;
